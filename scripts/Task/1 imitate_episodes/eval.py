"""Run a trained CNNMLP policy in joint-space sim_env."""
...
from Network.cnn_mlp import CNNMLP
...
def resolve_ckpt_path(cfg):
    ckpt_name = cfg.eval.get("eval_ckpt_name", None)
    ...
    if not ckpts:
        raise FileNotFoundError(
            ...
            "Train first or set eval.eval_ckpt_name in CNNMLP.yaml."
        )
    return ckpts[-1]
...
def save_gif(frames, path, fps):
    """Save RGB uint8 frames (list of HWC arrays) to an animated GIF."""
    if not frames:
        return
    path = Path(path)
...
def policy_action(policy, ts, camera_names, qpos_ns, action_ns):
    qpos = np.asarray(ts.observation.qpos, dtype=np.float32)
    qpos_t = normalize(torch.from_numpy(qpos), qpos_ns).cuda().unsqueeze(0)
    image_t = get_image(ts, camera_names)
    with torch.inference_mode():
        action_hat = policy(qpos_t, image_t)  # [1, dA]
    action = unnormalize(action_hat.squeeze(0).cpu(), action_ns).numpy()
    return action[:-2]  # drop dummy base_action -> 14-dim
...
def rollout(env, policy, cfg, qpos_ns, action_ns, onscreen=False, save_gif_path=None):
    ...
    gif_camera = cfg.eval.get("gif_camera", "top")
    gif_stride = int(cfg.eval.get("gif_stride", 1))
    episode_len = int(cfg.eval.episode_len)
    DT = float(cfg.sim.DT)
    ...
    for t in range(episode_len):
        ...
        action = policy_action(policy, ts, cfg.robot.camera_names, qpos_ns, action_ns)
        ts = env.step(action)
...
def main():
    cfg = load_cfg()
    onscreen = bool(cfg.eval.onscreen_render)
    num_rollouts = int(cfg.eval.num_rollouts)
    save_gif_flag = bool(cfg.eval.get("save_gif", False))
    ...
    gif_dir = Path(cfg.eval.get("gif_dir") or (P.ckpt_dir / "gifs"))
    if save_gif_flag:
        gif_dir.mkdir(parents=True, exist_ok=True)

    successes, returns, highest_rewards = [], [], []
    for i in tqdm(range(num_rollouts), desc="eval"):
        gif_path = None
        if save_gif_flag:
